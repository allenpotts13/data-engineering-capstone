INSERT INTO analysis.analysis_vpicdecode
SELECT
...
WHERE ST_CASE IS NOT NULL AND VEH_NO IS NOT NULL
    AND NOT EXISTS (
        SELECT 1 FROM analysis.analysis_vpicdecode a
        WHERE a.st_case = TRY_CAST(bronze.bronze_vpicdecode.ST_CASE AS INTEGER)
            AND a.veh_no = TRY_CAST(bronze.bronze_vpicdecode.VEH_NO AS INTEGER)
    );
