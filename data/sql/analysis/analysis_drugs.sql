INSERT INTO analysis.analysis_drugs
SELECT
...
WHERE ST_CASE IS NOT NULL AND VEH_NO IS NOT NULL AND PER_NO IS NOT NULL
    AND NOT EXISTS (
        SELECT 1 FROM analysis.analysis_drugs a
        WHERE a.st_case = TRY_CAST(bronze.bronze_drugs.ST_CASE AS INTEGER)
            AND a.veh_no = TRY_CAST(bronze.bronze_drugs.VEH_NO AS INTEGER)
            AND a.per_no = TRY_CAST(bronze.bronze_drugs.PER_NO AS INTEGER)
    );