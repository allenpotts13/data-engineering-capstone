INSERT INTO analysis.analysis_vevent
SELECT
...
WHERE ST_CASE IS NOT NULL AND VEH_NO IS NOT NULL AND EVENTNUM IS NOT NULL
    AND NOT EXISTS (
            SELECT 1 FROM analysis.analysis_vevent a
            WHERE a.st_case = TRY_CAST(bronze.bronze_vevent.ST_CASE AS INTEGER)
                AND a.veh_no = TRY_CAST(bronze.bronze_vevent.VEH_NO AS INTEGER)
                AND a.eventnum = TRY_CAST(bronze.bronze_vevent.EVENTNUM AS INTEGER)
    );
