INSERT INTO analysis.analysis_vpictrailerdecode
SELECT
...
WHERE ST_CASE IS NOT NULL AND VEH_NO IS NOT NULL AND TRAILER_NO IS NOT NULL
    AND NOT EXISTS (
            SELECT 1 FROM analysis.analysis_vpictrailerdecode a
            WHERE a.st_case = TRY_CAST(bronze.bronze_vpictrailerdecode.ST_CASE AS INTEGER)
                AND a.veh_no = TRY_CAST(bronze.bronze_vpictrailerdecode.VEH_NO AS INTEGER)
                AND a.trailer_no = TRY_CAST(bronze.bronze_vpictrailerdecode.TRAILER_NO AS INTEGER)
    );
