...
INSERT INTO silver.silver_pbtype
SELECT
...
WHERE ST_CASE IS NOT NULL AND VEH_NO IS NOT NULL AND PER_NO IS NOT NULL
    AND NOT EXISTS (
            SELECT 1 FROM silver.silver_pbtype s
            WHERE s.st_case = TRY_CAST(bronze.bronze_pbtype.ST_CASE AS INTEGER)
                AND s.veh_no = TRY_CAST(bronze.bronze_pbtype.VEH_NO AS INTEGER)
                AND s.per_no = TRY_CAST(bronze.bronze_pbtype.PER_NO AS INTEGER)
    );
