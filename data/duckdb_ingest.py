...
def list_fars_files(minio_client, bucket_name):
...
def is_file_ingested(con, file_path):
    result = con.execute(
        "SELECT 1 FROM config.ingest_config WHERE file_path = ?", [file_path]
    ).fetchone()
...
def download_and_clean_data(minio_client, bucket_name, file_path, schema_columns):
    ...
    extra_cols = [col for col in imported_data.columns if col not in schema_columns]
    if extra_cols:
    ...
    for col in imported_data.columns:
        if pd.api.types.is_integer_dtype(imported_data[col]) or (
            ...
            and pd.api.types.is_integer_dtype(type(imported_data[col].dropna().iloc[0]))
        ):
            imported_data[col] = pd.to_numeric(
                imported_data[col], errors="coerce"
            ).astype("Int64")
    return imported_data, schema_columns
...
def create_or_append_table(con, table_name, imported_data, first_file):
    ...
    if first_file:
        ...
        logger.info(f"Appended data to DuckDB table {table_name}")


def record_ingestion(con, file_path):
    con.execute(
        "INSERT INTO config.ingest_config (file_path, ingested_at) VALUES (?, CURRENT_TIMESTAMP)",
        [file_path],
    )
...
def fars_data_to_duckdb():
    ...
    file_type_to_files = list_fars_files(minio_client, bucket_name)
    ...
    for file_type, matched_files in file_type_to_files.items():
        ...
        for file_path in matched_files_sorted:
            if is_file_ingested(con, file_path):
                logger.info(f"Skipping already ingested file: {file_path}")
                continue
            try:
                logger.info(f"Downloading {file_path} from MinIO for DuckDB import")
                imported_data, schema_columns = download_and_clean_data(
                    minio_client, bucket_name, file_path, schema_columns
                )
                create_or_append_table(con, table_name, imported_data, first_file)
                record_ingestion(con, file_path)
                first_file = False
