...
def scrape_helmet_laws():
    ...
    state_names_path = os.path.join(
        os.path.dirname(__file__), "..", "config", "state_name.txt"
    )
    with open(state_names_path, "r") as f:
...
def main():
    ...
    minio_csv_path = upload_csv_to_minio(
        headers, helmet_laws, minio_client, bucket_name, csv_filename
    )
